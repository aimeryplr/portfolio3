...
export default function ProjectPreview({ project }: Props) {
  ...
  return (
    <div
      ...
      className="group relative flex flex-col gap-4 bg-surface overflow-hidden px-8 py-6 w-full box-border rounded-2xl transition-all duration-300 ease-out hover:scale-[1.01] hover:shadow-xl hover:shadow-primary/15 border-2 border-transparent hover:border-accent cursor-pointer"
    >
      ...
      <div
        ...
        style={{ maxHeight: showDesc ? "200px" : hovered ? "1.5em" : "0" }}
      >
        <p className={`text-surface-foreground ${showDesc ? "" : "line-clamp-1"}`}>{project.description}</p>
      </div>
